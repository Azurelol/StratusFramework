...
namespace Stratus.Gameplay
{
	...
	public class StratusCharacterAnimator : StratusManagedBehaviour
	{
		//------------------------------------------------------------------------/
		// Event Declarations
		//------------------------------------------------------------------------/
		...
		private void SetHooks()
		{
			foreach (var hook in animatorEventHooks)
			{
				...
				{
					StratusDebug.LogError($"An animator trigger has not been set properly!", this);
					continue;
				}
			...
			foreach (var hook in animatorParameterHooks)
			{
				...
				{
					StratusDebug.LogError($"An animator parameter has not been set properly!", this);
					continue;
				}
		...
		private void RecordParameters()
		{
			...
			if (this.animator)
			{
				...
				this._animatorParameterNames = this._animatorParameters.ToStringArray((AnimatorControllerParameter parameter) => parameter.name);
				this._floatParameters = this._animatorParameters.Where(x => x.type == AnimatorControllerParameterType.Float).ToArray().ToStringArray((AnimatorControllerParameter parameter) => parameter.name);
				this._intParameters = this._animatorParameters.Where(x => x.type == AnimatorControllerParameterType.Int).ToArray().ToStringArray((AnimatorControllerParameter parameter) => parameter.name);
				this._boolParameters = this._animatorParameters.Where(x => x.type == AnimatorControllerParameterType.Bool).ToArray().ToStringArray((AnimatorControllerParameter parameter) => parameter.name);
				this._triggerParameters = this._animatorParameters.Where(x => x.type == AnimatorControllerParameterType.Trigger).ToArray().ToStringArray((AnimatorControllerParameter parameter) => parameter.name);
			}
			else
			{
