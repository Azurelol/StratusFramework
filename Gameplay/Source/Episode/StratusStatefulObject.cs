...
namespace Stratus
{
  ...
  /// </summary>
  public class StratusStatefulObject : StratusBehaviour
  {
    //------------------------------------------------------------------------/
    // Declarations: Types
    //------------------------------------------------------------------------/
    ...
    {
      public List<StratusMemberReference> memberReferences;
    ...
    public class StateEvent : Stratus.StratusEvent
    {
      ...
      public bool usesLabel => type == StratusStatefulObject.EventType.Save || type == StratusStatefulObject.EventType.Load;
    }
    ...
    private List<StratusMemberReference> memberReferences = new List<StratusMemberReference>();
    ...
    public SerializedState SaveState(string label)
    {
      ...
      if (stateMap.ContainsKey(label))
      {
        ...
        if (debug)
          StratusDebug.Log($"The state {label} has been overwritten!", this);
      }
      // Else make a new one
      else
      {
    ...
    private void LoadState(SerializedState state)
    {
      ...
      if (debug)
        StratusDebug.Log($"Loaded the state {state.label} with {memberCount} members!", this);
    ...
    private SerializedState MakeState(string label)
    {
      ...
      if (debug)
        StratusDebug.Log($"Recorded the state {state.label} with {memberCount} members!", this);
