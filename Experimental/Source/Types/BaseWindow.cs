...
namespace Stratus
{
	...
	/// </summary>
	public abstract class BaseWindow : StratusBehaviour, IStratusDebuggable
	{
		//--------------------------------------------------------------------------------------------/
		// Declarations
		//--------------------------------------------------------------------------------------------/
		...
		public StratusInputBinding cancel = new StratusInputBinding();
		...
		public void Transition(bool show, System.Action onFinished = null)
		{
			...
			// Fade the canvas
			StratusActions.Property(currentSeq, () => canvas.alpha, show ? 1f : 0f, transitionSpeed, StratusEase.Linear);
	...
	public abstract class BaseWindow<T> : BaseWindow
	{
		...
		public static void Open(OpenEvent e, bool transition = false)
		{
			...
			if (transition && e.parent != null)
			{
				...
				StratusActions.Call(seq, () => StratusScene.Dispatch<OpenEvent>(e));
			}
			// Otherwise the child window right away
			else
			{
				StratusScene.Dispatch<OpenEvent>(e);
			}
		}
		...
		{
			StratusScene.Dispatch<OpenEvent>(new OpenEvent() { data = args, onClosed = onClosed });
		}

		public static void RequestClose(object[] args = null)
		{
			StratusScene.Dispatch<CloseEvent>(new CloseEvent());
		}
