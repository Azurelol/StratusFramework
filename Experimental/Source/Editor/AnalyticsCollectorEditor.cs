...
namespace Stratus.Analytics
{
  ...
  {
    private StratusDropdownList<Analysis.Attribute> attributes;
    ...
    private void MakeAttributeList()
    {
      ...
      attributes = new StratusDropdownList<Analysis.Attribute>(target.schema.attributes, (Analysis.Attribute attr) => $"{attr.label} ({attr.type}) ", target.attribute);
      //MakeCheckpointList();
