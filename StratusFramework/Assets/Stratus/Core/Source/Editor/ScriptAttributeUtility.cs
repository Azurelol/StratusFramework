...
namespace Stratus.Dependencies
{
  /// <summary>
  ...
  internal class ScriptAttributeUtility
  {
    ...
    private static SerializedPropertyHandler s_SharedNullHandler = new SerializedPropertyHandler();

    private static SerializedPropertyHandler s_NextHandler = new SerializedPropertyHandler();
    ...
    internal static SerializedPropertyHandler GetHandler(SerializedProperty property)
    {
      if (property == null)
      {
      ...
      SerializedPropertyHandler handler = propertyHandlerCache.GetHandler(property);
      if (handler != null)
      {
        return handler;
      }
      ...
      else
      {
        ...
        s_NextHandler = new SerializedPropertyHandler();
      }
      return handler;
    }
    ...
    {
      Type hiddenType = Reflection.GetPrivateType("UnityEditor.EditorAssemblies", typeof(CustomEditor));
      return (IEnumerable<Type>)Reflection.GetReflectedMethod("SubclassesOf", hiddenType).Invoke(null, new object[] { parent });
    }
  }
  ...
  {
    protected Dictionary<int, SerializedPropertyHandler> m_PropertyHandlers = new Dictionary<int, SerializedPropertyHandler>();

    internal SerializedPropertyHandler GetHandler(SerializedProperty property)
    {
      int propertyHash = GetPropertyHash(property);
      SerializedPropertyHandler result = null;
    ...
    internal void SetHandler(SerializedProperty property, SerializedPropertyHandler handler)
    {
      int propertyHash = GetPropertyHash(property);
