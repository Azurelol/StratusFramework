  ...
  public class EventBrowserWindow : StratusEditorWindow<EventBrowserWindow>
  {
    ...
    public class EventInformation : INamed
    {
      ...
      public EventInformation(System.Type type)
      {
        ...
        members.AddRange(type.GetFields(bindingFlags).Names((FieldInfo member) => $"({member.FieldType.Name}) {member.Name}"));
        members.AddRange(type.GetProperties(bindingFlags).Names((PropertyInfo member) => $"({member.PropertyType.Name}) {member.Name}"));
    ...
    private static void Open() => OnOpen("Event Browser");
