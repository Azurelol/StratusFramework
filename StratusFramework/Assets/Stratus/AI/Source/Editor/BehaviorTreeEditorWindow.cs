...
namespace Stratus
{
	namespace AI
	{
			...
			public class BehaviourTreeView : HierarchicalTreeView<BehaviorTree.BehaviorNode>
			{
				...
				protected override void OnItemContextMenu(GenericMenu menu, BehaviorTree.BehaviorNode treeElement)
				{
					...
					if (treeElement.data is Task)
					{
						...
						menu.AddItem("Duplicate", false, () => this.window.AddNode((Behavior)treeElement.data.Clone(), parent));

						menu.AddPopup("Add/Decorator", BehaviorTreeEditorWindow.decoratorTypes.displayedOptions, (int index) =>
						{
							this.window.AddParentNode(BehaviorTreeEditorWindow.decoratorTypes.AtIndex(index), treeElement);
						});
					}
					...
					else if (treeElement.data is Composite)
					{
						...
						{
							this.window.AddChildNode(BehaviorTreeEditorWindow.taskTypes.AtIndex(index), treeElement);
						});

						menu.AddPopup("Add/Composites", BehaviorTreeEditorWindow.compositeTypes.displayedOptions, (int index) =>
						{
							this.window.AddChildNode(BehaviorTreeEditorWindow.compositeTypes.AtIndex(index), treeElement);
						});
						...
						{
							this.window.ReplaceNode(treeElement, BehaviorTreeEditorWindow.compositeTypes.AtIndex(index));
				...
				protected override void OnContextMenu(GenericMenu menu)
				{
					...
					menu.AddPopup("Add/Composites", BehaviorTreeEditorWindow.compositeTypes.displayedOptions, (int index) =>
					{
						this.window.AddChildNode(BehaviorTreeEditorWindow.compositeTypes.AtIndex(index));
					});

					// Actions
					menu.AddPopup("Add/Tasks", BehaviorTreeEditorWindow.taskTypes.displayedOptions, (int index) =>
					{
						this.window.AddChildNode(BehaviorTreeEditorWindow.taskTypes.AtIndex(index));
					});
		...
		nameof(Mode.Debugger),
			...
			/// </summary>
			private Blackboard blackboard => this.behaviorTree.blackboard;
			...
			protected override void OnWindowEnable()
			{
				...
				else
				{
					...
					this.treeInspector = new BehaviourTreeView(this.treeViewState, treeBuilder.ToTree());
				}
			...
			private void DrawDebugger()
			{
				...
				this.EditObjectFieldWithHeader(ref this.selectedAgent, "Agent");
					...
					if (this.blackboardEditor != null)
					{
						...
						switch (this.scope)
						{
							case Blackboard.Scope.Local:
								this.blackboardEditor.DrawSerializedProperty(nameof(Blackboard.locals));
								break;
							case Blackboard.Scope.Global:
								this.blackboardEditor.DrawSerializedProperty(nameof(Blackboard.globals));
								break;
						}
			...
			private void OnSelectionChanged(IList<int> ids)
			{
				...
				if (this.currentNodes.Count > 0)
				{
					...
					this.currentNodeSerializedObject = new SerializedSystemObject(this.currentNode.data);
